...
DROP SEQUENCE seq_cod_instrumento;
...
CREATE SEQUENCE seq_cod_instrumento
    START WITH 100
    INCREMENT BY 1
    NOCACHE
    NOCYCLE;
...
CREATE TABLE Banda (
    ...
    CONSTRAINT chk_banda_genero CHECK (genero IN (
        'Rock', 'Pop', 'Sertanejo', 'Forró', 'MPB', 'Jazz', 'Blues',
        'Eletrônico', 'Hip-Hop', 'Reggae', 'Metal', 'Punk', 'Clássico', 'Outro'
    ))
);
...
CREATE TABLE Instrumento (
    membro      CHAR(11)       NOT NULL,
    cod_inv     NUMBER(10)     DEFAULT seq_cod_instrumento.NEXTVAL NOT NULL,
    ...
    CONSTRAINT chk_instrumento_tipo CHECK (tipo IN (
        'Guitarra', 'Baixo', 'Bateria', 'Teclado', 'Violão', 'Contrabaixo',
        'Saxofone', 'Trompete', 'Flauta', 'Violino', 'Percussão', 'Outro'
    ))
);
...
CREATE TABLE Equipamento_de_palco (
    ...
    CONSTRAINT chk_equip_tipo CHECK (tipo IN (
        ...
        'Monitor de palco', 'Iluminação LED', 'Gerador', 'Cabo P10',
...
CREATE TABLE Apresentacao (
    ...
    publico_presente    NUMBER(10)      NOT NULL,
    cache_combinado     NUMBER(12, 2)   NOT NULL,
    ...
    CONSTRAINT chk_apres_publico CHECK (publico_presente >= 0),

    CONSTRAINT chk_apres_cache CHECK (cache_combinado >= 0)
);
